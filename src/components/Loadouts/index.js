...
class Loadouts extends Component {
  ...
  getPiecesFor = (piece) => {
    ...
    fetch(`https://mhw-db.com/armor/?q={"type": "${piece}"}`, {headers: {'Accept': 'application/json', 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'}})
  ...
  getSkills = () => {
    if (this.state.skillsIsFetching === false && this.state.skillsIsFetched === false) {
      ...
      let skills = [];
      fetch(`https://mhw-db.com/skills/`, {headers: {'Accept': 'application/json', 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'}})
        .then(response => response.json())
        .then(responseData => {
