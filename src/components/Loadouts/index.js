...
class Loadouts extends Component {
  ...
  getPiecesFor = (piece) => {
    ...
    fetch(`https://mhw-db.com/armor/?q={"type": "${piece}"}`, {headers: {'Accept': 'application/json', 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': 'https://mhw-db.netlify.com/'}})
  ...
  getSkills = () => {
    if (this.state.skillsIsFetching === false && this.state.skillsIsFetched === false) {
      ...
      let skills = [];
      fetch(`https://mhw-db.com/skills/`, {headers: {'Accept': 'application/json', 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': 'https://mhw-db.netlify.com/'}})
        .then(response => response.json())
        .then(responseData => {
